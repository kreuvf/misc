...
# better.
#
# Effect (MW + CodeMirror + intersection + Maintenance)
# 17,730 items (160.7 MB) -> 6,214 items (38.1 MB)
#
...
find ./"$1"/ -type f -regextype posix-extended -regex '.*(\.editorconfig|\.(es|style)lintrc\.json|\.git(ignore|review)|\.scrutinizer\.yml|\.stylelintrc|(APACHE|GPL|MIT)?-?LICENSE-?(2\.0|MIT)?(\.txt)?|AUTHORS(\.txt)?|changelog\.txt|CHANGELOG(\.md)?|CHANGES\.md|CODE_OF_CONDUCT\.md|composer\.local\.json-sample|CONTRIBUTING\.md|COPYING(\.txt)?|CREDITS|dependencies\.txt|Doxyfile|errors\.md|example_debug\.png|FAQ|formats\.md|Gemfile\.lock|gitinfo\.json|Gruntfile\.js|History(\.md)?|HISTORY(\.md)?|INSTALL|Makefile(\.py)?|license\.txt|NOTICE\.txt|PATCHES|\.patch|\.?php_?cs(\.xml)?|phpdoc\.sh|Rakefile|readme\.txt|README(\.markdown|\.md|\.rst)?|RELEASE-NOTES-?.*|SECURITY|StartProfiler\.sample|structure\.txt|styleguide\.md|UPGRADE(\.md)?|version|resources/lib/jquery\.i18n/CREDITS)$' | xargs /bin/rm -f
